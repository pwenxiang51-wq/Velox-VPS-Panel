 ...
 8)
        ...
        if [[ "$argo_action" == "1" ]]; then
            ...
            if [ -n "$argo_token" ]; then
                ...
                systemctl stop cloudflared argo >/dev/null 2>&1
                systemctl disable cloudflared argo >/dev/null 2>&1
...
⏰ 北京时间: $(date +'%Y-%m-%d %H:%M:%S')"
        ...
        if [ -n "$MAIN_IF" ]; then
            curl --interface "$MAIN_IF" -s -X POST "https://api.telegram.org/bot${GLOBAL_TG_TOKEN}/sendMessage" --data-urlencode chat_id="${GLOBAL_TG_CHATID}" --data-urlencode text="$MSG" > /dev/null 2>&1 &
        else
            curl -s -X POST "https://api.telegram.org/bot${GLOBAL_TG_TOKEN}/sendMessage" --data-urlencode chat_id="${GLOBAL_TG_CHATID}" --data-urlencode text="$MSG" > /dev/null 2>&1 &
        fi
    fi
fi
EOF2
...
⏰ 北京时间: $(date +'%Y-%m-%d %H:%M:%S')"
MAIN_IF=$(ip -4 route ls | grep default | grep -v tun | grep -v warp | grep -v wg | awk '{print $5}' | head -n 1)
if [ -n "$MAIN_IF" ]; then curl --interface "$MAIN_IF" -s -X POST "https://api.telegram.org/bot${GLOBAL_TG_TOKEN}/sendMessage" --data-urlencode chat_id="${GLOBAL_TG_CHATID}" --data-urlencode text="$MSG" > /dev/null 2>&1
else curl -s -X POST "https://api.telegram.org/bot${GLOBAL_TG_TOKEN}/sendMessage" --data-urlencode chat_id="${GLOBAL_TG_CHATID}" --data-urlencode text="$MSG" > /dev/null 2>&1; fi
EOF2
...
EOF_P
                    ...
                    3)
                        ...
                        echo -e "\n${cyan}正在向系统底层注入防猝死秒级监控探针...${plain}"
                        cat << 'EOF_WATCH' > /usr/local/bin/velox_watchdog.sh
#!/bin/bash
source /etc/velox_tg.conf
PROCESS_NAME="sing-box"
FLAG_FILE="/tmp/${PROCESS_NAME}_dead.flag"
...
--------------------------------------
🖥️ <b>阵地:</b> <code>$(hostname)</code>
⚠️ <b>状态:</b> <code>${PROCESS_NAME}</code> 核心已阵亡！
...
--------------------------------------
🖥️ <b>阵地:</b> <code>$(hostname)</code>
✅ <b>状态:</b> <code>${PROCESS_NAME}</code> 核心已重新归队！
⏰ <b>时间:</b> $(date +'%Y-%m-%d %H:%M:%S')
--------------------------------------
<i>(雷达侦测：系统已恢复正常运作)</i>"
        curl -s -X POST "https://api.telegram.org/bot${GLOBAL_TG_TOKEN}/sendMessage" -d "chat_id=${GLOBAL_TG_CHATID}" -d "text=$MSG" -d parse_mode="HTML" > /dev/null 2>&1
...
EOF_WATCH
                        ...
                        echo -e "\n${green}✅ 部署成功！秒级哨兵已潜入后台，Sing-box 敢死秒报警，复活秒通知！${plain}"
                        read -p "👉 按【回车键】继续..."
                        ;;
                    # -------------------------------------------

                    # --- 嵌入 6：修改选项 4 (包含哨兵清理逻辑) ---
                    4)
                        if [ ! -f "/usr/local/bin/ssh_tg_alert.sh" ] && ! crontab -l 2>/dev/null | grep -qE "velox_pulse_alert.sh|velox_watchdog.sh"; then
                            ...
                            sudo rm -f /usr/local/bin/ssh_tg_alert.sh /usr/local/bin/tg_boot_alert.sh /etc/systemd/system/tg_boot_alert.service /usr/local/bin/velox_pulse_alert.sh /usr/local/bin/velox_watchdog.sh /tmp/sing-box_dead.flag
